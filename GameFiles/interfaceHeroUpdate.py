...
def equipAndDrop(hero, item):
    ...
    if(item.item_type == "armor"):
        ...
        hero.player_class = playerClass(item)
        hero.weapon = item
...
def increaseXP(amount, hero):
    ...
    if(hero.experience >= (10+(hero.player_level ** 2))):
        ...
        hero.player_level += 1
        hero.points += 1
        text2Print.insert(INSERT, "You have leveled up! You are now level: " + str(hero.player_level) + "\n")
    save(hero)
...
def add_points(hero, text2Print, userEntry):
    ...
    while (attribute not in accepted):
        ...
        attribute = userEntry.get()
        print(attribute)
        try:
            attribute = accepted_dict[int(attribute)]
        except:
            text2Print.insert(INSERT, "Input was invalid. Please enter either an attribute or its corresponding number \n")
            print('except')
    ...
    while type(amount) != int or amount > hero.points:
        try:
            text2Print.insert(INSERT, "By how much? \n")
            amount = int(userEntry.get())
            assert amount <= hero.points
        except AssertionError:
            text2Print.insert(INSERT, "You do not have that many points remaining \n")
        except:
            text2Print.insert(INSERT, "You must enter an integer amount \n")
